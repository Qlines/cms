...
<%
    ...
    String [][]menu = {
        {"OVERVIEW",        "overview.jsp",         "<img style=\"vertical-align:middle\" src=\"./images/info32.png\" border=\"0\">Overview",         USER_TYPE.ADMIN.toString(),        "Home"},
        {"ADMIN_SETTINGS",  null,                   "<img style=\"vertical-align:middle\" src=\"./images/process32.png\" border=\"0\">Admin Settings",   USER_TYPE.USER.toString(),         "padminsettings"},
        {"SERVICES",        "services_search.jsp",  "Services",         USER_TYPE.ADMIN.toString(),        "Admin Settings > Services"},
        {"LINKS",           "link.jsp",             "Links",            USER_TYPE.ADMIN.toString(),        "Admin Settings > Links"},
        //{"MIGRATESUB",      "migrateSub.html",      "Migrate Sub Tool", USER_TYPE.ADMIN.toString(),        "Admin Settings > Migrate Sub Tool"},
        //{"NUMBERPLAN",      "number_plan.html",     "Numbering Plan",   USER_TYPE.ADMIN.toString(),        "Admin Settings > Numbering Plan"},
        {"MEMBER",          "member.jsp",           "Member",           USER_TYPE.USER.toString(),         "Admin Settings > Member"},
        {"LOG_EVENT",       "log_event.jsp",        "Log Event",        USER_TYPE.ADMIN.toString(),        "Admin Settings > Log Event"},
        {null,              null,                   "ADMIN_SETTINGS",   USER_TYPE.USER.toString(),         null}, // ADMIN_SETTINGS
        {"PUSH_MESSAGE",    "push_message_sms.jsp", "<img style=\"vertical-align:middle\" src=\"./images/mail_send32.png\" border=\"0\">Push Message",     USER_TYPE.USER.toString(),         "Push Message"},
        {"CONTENT_SETTINGS",null,                   "<img style=\"vertical-align:middle\" src=\"./images/page_process32.png\" border=\"0\">Content Settings", USER_TYPE.ADMIN.toString(),        "pcontentsettings"},
        {"SMS",             null,                   "SMS",              USER_TYPE.ADMIN.toString(),        "psms"},
        {"SMS_TEXT",        "sms_text_search.jsp",  "Text Message",     USER_TYPE.ADMIN.toString(),        "Content Settings > SMS > Text Message"},
        {"SMS_WAP",         "sms_wap_search.jsp",   "WAP Push",         USER_TYPE.ADMIN.toString(),        "Content Settings > SMS > WAP Push"},
        //{"SMS_RINGTONE",    "sms_ringtone_search.jsp",     "Ringtone",  USER_TYPE.ADMIN.toString(),        "Content Settings > SMS > Ringtone"},
        //{"SMS_PICTURE",     "sms_picture_search.jsp",  "Picture",          USER_TYPE.ADMIN.toString(),        "Content Settings > SMS > Picture"},
        {null,              null,                   "SMS",              USER_TYPE.ADMIN.toString(),        null}, // SMS
        //{"MMS",             "mms_search.jsp",       "MMS",              USER_TYPE.ADMIN.toString(),        "Content Settings > MMS"},
        {"THIRDPARTY",      "thirdparty_search.jsp","3rd Party",        USER_TYPE.ADMIN.toString(),        "Content Settings > 3rd Party"},
        {"CONTENTMAP",      "services_content_map.jsp","Content Map",        USER_TYPE.USER.toString(),        "Content Settings > Content Map"},
        {null,              null,                   "CONTENT_SETTINGS", USER_TYPE.ADMIN.toString(),        null}, // CONTENT_SETTINGS
        {"BLOCK_LIST",      "block_list.jsp",       "<img style=\"vertical-align:middle\" src=\"./images/block32.png\" border=\"0\">Blocked List",
                                                                        USER_TYPE.SENIOR.toString(),       "Blocked List"},
        {"REPORT",          null,                   "<img style=\"vertical-align:middle\" src=\"./images/chart_pie32.png\" border=\"0\">Report",           USER_TYPE.USER.toString(),         "preport"},
        {"REPORT_SUBSCRIPTION",
                            "report_subscription.jsp", "Subscription",  USER_TYPE.USER.toString(),         "Report > Subscription"},
        {"REPORT_SMSDOWNLOAD",
                            "report_smsdownload.jsp", "SMS Download",   USER_TYPE.USER.toString(),         "Report > SMS Download"},
        {null,              null,                   "REPORT",           USER_TYPE.USER.toString(),        null}, // REPORT
...
<%
    ...
    for (int i=0;i<menu.length;i++) {
        if (policy.getId() <= USER_TYPE.valueOf(menu[i][3]).getId() && menu[i][1] == null && menu[i][0] != null) {
            out.println("var " + menu[i][4] + " = new menuObj('" + menu[i][0].toLowerCase() + "', null);");
            expd_menu += menu[i][4] + ",";
        }
    }
    if (!expd_menu.isEmpty()) {
        expd_menu = expd_menu.substring(0, expd_menu.length()-1);
    }
%>
...
<%
    String hint = "";
    for (int i=0;i<menu.length;i++) {
        if (policy.getId() <= USER_TYPE.valueOf(menu[i][3]).getId() && menu[i][0] != null) {
            hint += "'" + menu[i][0] + "','',";
        }
    }
    if (!hint.isEmpty()) {
...
<%
    ...
    for (int i=0;i<menu.length;i++) {
        if (policy.getId() <= USER_TYPE.valueOf(menu[i][3]).getId() && menu[i][0] != null) {
            out.println("case " + j + ":");
            out.println("obj.onmousemove = function(){");
            out.println("showHintBox('" + menu[i][0] + "_hint')};");
            out.println("obj.onmouseout = function(){");
            out.println("hideHintBox('" + menu[i][0] + "_hint')};");
            out.println("break;");
            j++;
        }
    }
...
function changeText(id, txt) {
    obj = getElement(id);
    if (txt != 'Home')
        txt = '<a href="javascript:changeText(\'sitemap\', \'Home\');open_page(\'<%=group.getHomepage()%>\', ' + framename + ');" style="text-decoration:none;">Home</a> &gt; ' + txt;
    obj.innerHTML = txt;
}
...
<%
    ...
    expd_menu = "";
    for (int i=0;i<menu.length;i++) {
        if (policy.getId() <= USER_TYPE.valueOf(menu[i][3]).getId()) {
            if (menu[i][0] != null && menu[i][1] == null) { // expand menu
                ndiv++;
                expd_menu += "+ '<a id=" + menu[i][0] + " ' + getMenuStat(" + menu[i][4] + ") + ' href=\"javascript:setMenuStat(" + menu[i][4] + ", null);void(0)\">" + menu[i][2] +"</a>'\n";
                expd_menu += "+ '<div id=\"" + menu[i][0].toLowerCase() + "\">'\n";
            } else if (menu[i][0] != null && menu[i][1] != null && group.isAllowPages(menu[i][1]) && !group.isBlockPages(menu[i][1])){ // sub menu
                if (!expd_menu.isEmpty()) {
                    out.print(expd_menu);
                    expd_menu = "";
                ...
                out.println("+ '<a id=" + menu[i][0] + " href=\"javascript:changeText(\\'sitemap\\', \\'" + menu[i][4] + "\\');open_page(\\'" + menu[i][1] + "\\', ' + framename + ');\">" + menu[i][2] + "</a>'");
            } else if (menu[i][0] == null && menu[i][1] == null) { // end of div
                expd_menu = "";
                ndiv--;
                try {
                    out.println(div.pop());
...
document.write('  <a id=LOGOUT href="javascript:window.location=\'logout\'"><img style=\"vertical-align:middle\" src=\"./images/lock32.png\" border=\"0\">Logout</a>');
document.write('</div>');
