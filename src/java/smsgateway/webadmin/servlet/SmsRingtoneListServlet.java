...
public class SmsRingtoneListServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (user == null) {
                ...
                File dir = new File("ext/sms/ringtone");
                if ((!dir.exists())
                        && (!dir.mkdirs())) {
                    throw new Exception("cannot create ringtone sms directory");
                }

                if ((request.getContentType() != null) && (request.getContentType().contains("multipart/form-data"))) {
                    MultipartRequest multi = new MultipartRequest(request, "ext/sms/ringtone", 5242880);
                    ...
                    while (files.hasMoreElements()) {
                        ...
                        try {
                            new MessageSms().add("ext/sms/ringtone/" + filename, Message.SMS_TYPE.RINGTONE, 0, multi.getFile(name), user.getUid());
                        } catch (Exception e) {
                        }
                    }
                } else {
                ...
                if ((cmd != null) && (cmd.equals("remove"))) {
                    for (String id : sid.split(",")) {
                        try {
                            ...
                            System.out.println("Remove sms ringtone " + sms.remove() + " row(s)");
                        } catch (Exception e) {
                        }
                    }
                ...
                if ((cmd != null) && (cmd.equals("refresh"))) {
                    ...
                    request.getSession().setAttribute("smsRingtoneList", new MessageSmsFactory(Message.SMS_TYPE.RINGTONE, keyword, 0, 0, user.getUid()).getMessageSmsList());
                }

                List list = (List) request.getSession().getAttribute("smsRingtoneList");
                ...
                for (int i = sindex; (i < list.size()) && (i < eindex); i++) {
                    try {
                        ...
                        out.println("<td" + ((request.getParameter("type") != null) && (!request.getParameter("type").isEmpty()) && (request.getParameter("type").equals("popup")) ? " onclick='javascript:window.returnValue=" + ((MessageSms) list.get(i)).getContent_id() + ";window.close();'" : new StringBuilder().append(" onclick='javascript:doSwapSelect(").append(((MessageSms) list.get(i)).getContent_id()).append(")'").toString()) + " id='td_" + ((MessageSms) list.get(i)).getContent_id() + "' " + " style='line-height:90%;'" + " onmouseover='javascript:_onMouseOver(this, \"" + ((MessageSms) list.get(i)).getContent_id() + "\");'" + " onmouseout='javascript:_onMouseOut(this, \"" + ((MessageSms) list.get(i)).getContent_id() + "\");'" + ">" + "<img src='images/play.gif' border=0" + " style='vertical-align:middle;'" + " onclick='doPlay(\"playerDiv_" + ((MessageSms) list.get(i)).getContent_id() + "\", \"header?file=" + ((MessageSms) list.get(i)).getFilename() + "\");'" + "><span id='rt_" + ((MessageSms) list.get(i)).getContent_id() + "'>" + ((MessageSms) list.get(i)).getFilename().substring(((MessageSms) list.get(i)).getFilename().lastIndexOf('/') + 1).trim() + "</span><br>" + "<input type='checkbox' id='" + ((MessageSms) list.get(i)).getContent_id() + "'" + " onclick='this.checked = !this.checked;'>" + "<div id='playerDiv_" + ((MessageSms) list.get(i)).getContent_id() + "'" + " style='width:100%;line-height:.8em;display:none;position:relative;left:0;top:0;background:transparent;padding:0;margin:0;font-size:75%;'>" + "</div>" + "</td>");
                ...
                out.println("</table></div><br><div>" + (subcmd.equals("select") ? "" : "<input type=button class='button' value='Select All' onclick='doSelectAll();'> <input type=button class='button' value='Deselect All' onclick='doDeselectAll();'> <input type=button class='button' value='Delete' onclick='return doRemove();'></div>") + "</body></html>");
            }
